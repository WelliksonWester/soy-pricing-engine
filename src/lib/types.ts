...
export const calculatorSchema = z.object({
  ...
  icmsOleo: z.coerce.number({ required_error: 'ICMS Óleo é obrigatório.' }).min(0, 'ICMS Óleo deve ser no mínimo 0.').max(100, 'ICMS Óleo não pode ser maior que 100.'),
  custoIcmsOleo: z.coerce.number().optional().default(0),
  precoBase: z.coerce.number({ required_error: 'Preço base é obrigatório.' }).min(0, 'Preço base deve ser maior ou igual a zero.'),
  ...
  custoFinanceiro: z.coerce.number().optional().default(0),
